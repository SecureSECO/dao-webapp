...
export const TokenSelectorDialogButtons = ({
  ...
  const filteredDaoBalances = daoBalanceData.daoBalances.filter(
    (token) =>
      !anyNullOrUndefined(
        token.name,
        token.symbol,
  ...
  return (
    <>
      {filteredDaoBalances.map((token, index) => (
        <div key={index} className="flex flex-row">
          <DialogClose
            key={index}
            type="button"
            className="h-10 rounded bg-slate-100 px-4 py-2 text-slate-900 hover:bg-slate-200 focus:ring-primary-200 dark:bg-slate-700 dark:text-slate-100 dark:hover:bg-slate-700/50 dark:focus:ring-primary-400"
